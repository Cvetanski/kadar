    ...
    <div class="py-6">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <style>
                ...
                .pr-grid{display:grid;grid-template-columns:1fr;gap:22px;max-width:900px;margin:16px auto 0;}
                @media (min-width:768px){ .pr-grid{grid-template-columns:1fr 1fr;} }

                .pr-card{
                    position:relative;background:#fff;border:1px solid #E8EBF0;border-radius:22px;padding:26px 30px 26px;
                ...
                .pr-card.pr-card-free:before{background:#D8DDE5;}
                .pr-card-name{font-size:13px;font-weight:800;color:#0958B5;text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px;}
                .pr-card-free .pr-card-name{color:#666B76;}
                .pr-pro-badge{display:inline-block;background:linear-gradient(135deg,#2D82E8,#0958B5);color:#fff;font-weight:800;padding:2px 9px;border-radius:999px;margin:0 2px;box-shadow:0 2px 6px rgba(11,111,224,.3);}
                .pr-card-tagline{font-size:13px;color:#9AA0AB;margin-bottom:16px;line-height:1.5;}
                .pr-card-price{font-size:40px;font-weight:900;color:#14171F;letter-spacing:-0.02em;line-height:1;}
                .pr-card-price span{font-size:15px;font-weight:600;color:#9AA0AB;}
                .pr-card-sub{font-size:12.5px;color:#9AA0AB;margin:6px 0 18px;}
                .pr-card-features{list-style:none;margin:0 0 20px;padding:0;text-align:left;}
                .pr-card-features li{display:flex;align-items:flex-start;gap:10px;font-size:13.5px;color:#333A44;padding:6px 0;
                ...
                .pr-card-free .pr-check{background:#9AA0AB;}
                ...
                .pr-card-btn.pr-card-btn-outline{
                    background:#fff;color:#14171F;border:1px solid #E8EBF0;box-shadow:none;
                }
                .pr-card-btn.pr-card-btn-outline:hover{color:#14171F;transform:none;box-shadow:none;}
                .pr-card-btn:disabled{
                    background:#F1F3F6;color:#9AA0AB;cursor:default;box-shadow:none;
                }
                .pr-card-reassure{font-size:11.5px;color:#9AA0AB;margin-top:14px;}
            </style>

            <div x-data="{ period: 'monthly', role: '{{ $defaultRole }}' }">
                <div class="pr-intro">
                ...
                @if (! $lockedRole)
                    <div class="pr-toggle">
                        <div class="pr-toggle-track">
                            <button type="button" class="pr-toggle-btn" :class="role === 'client' ? 'is-active' : ''" @click="role = 'client'">
                                For Clients
                            </button>
                            <button type="button" class="pr-toggle-btn" :class="role === 'creator' ? 'is-active' : ''" @click="role = 'creator'">
                                For Creators
                            </button>
                        </div>
                    </div>
                ...
                @foreach ($plans as $planKey => $plan)
                    @continue($lockedRole && $planKey !== $lockedRole)

                    <div x-show="role === '{{ $planKey }}'" @if(! $lockedRole) style="display:none;" @endif>
                        <div class="pr-grid">
                            <div class="pr-card pr-card-free">
                                <div class="pr-card-name">{{ $freePlans[$planKey]['name'] }}</div>
                                <p class="pr-card-tagline">{{ $freePlans[$planKey]['tagline'] }}</p>

                                <div class="pr-card-price">$0</div>
                                <p class="pr-card-sub">Forever free</p>

                                <ul class="pr-card-features">
                                    @foreach ($freePlans[$planKey]['features'] as $feature)
                                        <li>
                                            <span class="pr-check">
                                ...
                                @auth
                                    <button type="button" class="pr-card-btn pr-card-btn-outline" disabled>Current Plan</button>
                                @else
                                    <a href="{{ route('register', ['role' => $planKey]) }}" class="pr-card-btn pr-card-btn-outline">Get Started</a>
                                @endauth
                            </div>

                            <div class="pr-card">
                                <div class="pr-card-name">{!! str_ireplace('Pro', '<span class="pr-pro-badge">PRO</span>', $plan['name']) !!}</div>
