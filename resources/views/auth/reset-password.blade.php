...
<title>{{ __('Нова лозинка') }} — CreatorSpot</title>
...
<div class="wrap">
  <div class="card">
    <h1>{{ __('Постави нова лозинка') }}</h1>
    <p class="sub">{{ __('Внеси нова лозинка за твојата сметка.') }}</p>
    ...
    <form method="POST" action="{{ route('password.store') }}">
      ...
      <label for="password">{{ __('Нова лозинка') }}</label>
      ...
      <label for="password_confirmation">{{ __('Потврди лозинка') }}</label>
      <input type="password" id="password_confirmation" name="password_confirmation" placeholder="••••••••"
        class="{{ $errors->get('password_confirmation') ? 'has-error' : '' }}" required autocomplete="new-password">
      @error('password_confirmation')
        <div class="field-error">{{ $message }}</div>
      @enderror

      <button type="submit" class="btn-submit">{{ __('Зачувај нова лозинка') }}</button>
    </form>
  </div>
</div>
