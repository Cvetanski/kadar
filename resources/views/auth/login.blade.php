...
<title>{{ __('Најави се') }} — CreatorSpot</title>
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .row-between{display:flex;justify-content:space-between;align-items:center;margin-top:16px;font-size:13px;}
  ...
  .footer-link{text-align:center;font-size:14px;color:var(--text-dim);margin-top:8px;}
...
<div class="wrap">
  <div class="card">
    <h1>{{ __('Добредојде назад') }}</h1>
    <p class="sub">{{ __('Најави се и најди работа/креативец') }}</p>
    ...
    <form method="POST" action="{{ route('login') }}">
      ...
      <input type="password" id="password" name="password" placeholder="••••••••"
        class="{{ $errors->get('password') ? 'has-error' : '' }}" required autocomplete="current-password">
      @error('password')
        <div class="field-error">{{ $message }}</div>
      @enderror

      <div class="row-between">
        <label class="remember" style="margin:0;font-weight:400;">
          <input type="checkbox" name="remember" style="width:auto;"> {{ __('Запомни ме') }}
        </label>
        @if (Route::has('password.request'))
          <a href="{{ route('password.request') }}" class="forgot">{{ __('Заборавена лозинка?') }}</a>
        @endif
      </div>

      <button type="submit" class="btn-submit">{{ __('Најави се') }}</button>
    </form>

    <p class="footer-link">{{ __('Немаш акаунт?') }} <a href="{{ route('register') }}">{{ __('Регистрирај се бесплатно') }}</a></p>
  </div>
</div>
