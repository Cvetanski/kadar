{{--
    Override of Livewire's default "tailwind" pagination view. The stock view
    only shows numbered page links on sm: and above — on mobile it falls back
    to plain "Previous"/"Next" text pulled from the pagination.php lang file,
    which this app never published, so the raw "pagination.previous" /
    "pagination.next" translation keys rendered literally on phones. This
    version always shows the standard numbered pagination, no lang file
    needed since it relies only on icons + page numbers.
--}}
@php
if (! isset($scrollTo)) {
    $scrollTo = 'body';
