            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('projects.update', $project) }}"
                    ...
                    <div class="mt-2">
                        ...
                        <x-text-input id="title" name="title" class="block mt-1 w-full" type="text" :value="old('title', $project->title)" x-ref="titleField" />
                    ...
                    @if ($skillsByCategory->isNotEmpty())
                        <div class="mt-4" x-show="categoryIds.length > 0" x-cloak>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                        <div x-show="! budgetNegotiable">
                            ...
                            <x-text-input id="budget_min" name="budget_min" class="block mt-1 w-full" type="number"
                                step="0.01" min="0" :value="old('budget_min', $project->budget_min)" x-bind:disabled="budgetNegotiable" />
                        ...
                        <div x-show="! budgetNegotiable">
                            ...
                            <x-text-input id="budget_max" name="budget_max" class="block mt-1 w-full" type="number"
                                step="0.01" min="0" :value="old('budget_max', $project->budget_max)" x-bind:disabled="budgetNegotiable" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="deadline" name="deadline" class="block mt-1 w-full" type="date"
                    ...
                    <div class="mt-4" x-show="! remoteOk">
                        ...
                        <select id="country_id" name="country_id" x-model.number="countryId" @change="cityId = null"
                            ...
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}" {{ (old('country_id') ?: $project->country_id) == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex justify-between items-center mt-6">
                        <a href="{{ route('projects.show', $project) }}" class="text-sm text-gray-500 underline">{{ __('Откажи') }}</a>
                        <x-primary-button>{{ __('Зачувај промени') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
